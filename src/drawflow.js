...
load() {
    ...
    load()
    =======
    The load() function is responsible for initializing the drawflow editor by importing saved nodes and connections into the 
    canvas and updating the node ID for future additions.

    1. Node Import:         Loops through each node in the current module (this.module) and adds them to the 
                            canvas using addNodeImport. This reconstructs the editor's state by loading previously saved nodes.
    2. Reroute Handling:    If rerouting (alternative paths between nodes) is enabled (this.reroute), it goes 
                            through the node data again and calls addRerouteImport to manage reroute-specific rendering 
                            for each node.
    3. Connection Update:   The function updates the connections between nodes by calling updateConnectionNodes. This 
                            ensures that once the nodes are loaded, their connections (wires) are also rendered on the 
                            canvas.
    4. Node ID Management:  It scans all modules and their nodes to ensure that when a new node is created, it gets a 
                            unique ID. The highest node ID found is used as a reference, and this.nodeId is updated to 
                            the next available ID, ensuring no conflicts with existing nodes.
...
click(e) {
    ...
    click(e)
    =========
    The click(e) function is the core event handler for handling click (or touch) interactions in the Drawflow editor. 
    Depending on the context of what the user clicks (node, connection, or canvas area), it updates the state of the 
    editor by selecting/deselecting elements, handling dragging, drawing connections, and dispatching related events.

    1. Click Event Dispatch:The function begins by dispatching a 'click' event to notify the system that a click has occurred. 
                            At the end of the function, a 'clickEnd' event is also dispatched.
    2. Editor Modes:        The behavior of the function is dictated by the current editor_mode:
                                a) Fixed Mode: Only allows interactions with the canvas background.
                                b) View Mode: Allows interaction with the content, but not for editing.
                                c) Edit Mode: Fully interactive, allowing selection, dragging, and connection creation.
    3. Element Type Handling:The click behavior changes depending on the class of the clicked element:
                                a) drawflow-node: Handles node selection and dragging.
                                b) output: Initiates connection creation from the node's output.
                                c) main-path: Handles connection path selection.
                                d) drawflow-delete: Deletes nodes or connections.
                                e) point: Handles draggable points on the connection path.
                                f) Canvas area (drawflow, parent-drawflow): Handles deselection of nodes and connections.

    4. Node and Connection Management: Nodes and connections are selected, deselected, and updated as needed. This 
                            includes toggling visual cues like "selected" classes and dispatching relevant events 
                            (e.g., 'nodeSelected', 'nodeUnselected', 'connectionSelected').
    5. Touch Support:       The function accounts for both mouse and touch events, storing the appropriate positions 
